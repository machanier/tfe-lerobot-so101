#!/usr/bin/env python3
"""
check_calibration.py — validation complète de la chaîne de calibration.

Vérifie d'un coup :
  - calibration moteur (course, plage encodeur, conformité URDF) ;
  - calibration intrinsèque des 3 caméras (erreurs de reprojection) ;
  - calibration extrinsèque (hand-eye) des 3 caméras (positions, résidus) ;
  - cohérence inter-caméras (baseline stéréo cam_0 ↔ cam_1) ;
  - self-tests des modules cinématiques (transforms + FK).
...
def check_intrinsics():
    section("CALIBRATIONS INTRINSÈQUES (caméras)")
    all_ok = True
    for i in [0, 1, 2]:
        ...
        K = c["camera_matrix"]
        fx, fy = K[0][0], K[1][1]
        verdict = "OK" if err < 0.5 else ("acceptable" if err < 1.0 else "DÉGRADÉ")
...
def check_motor():
    ...
    try:
        ...
        return False
    # On réaffiche la sortie utile (l'en-tête est déjà imprimé plus haut).
...
def check_extrinsics():
    section("CALIBRATIONS HAND-EYE (extrinsèques)")
    all_ok = True
    cams = {}
    for i in [0, 1, 2]:
        ...
        print(f"      résidu translation    : moyen {res['translation_mean_dev_mm']:5.2f} mm | "
              f"max {res['translation_max_dev_mm']:5.2f} mm")
        print(f"      résidu rotation       : moyen {res['rotation_mean_dev_deg']:5.2f} deg | "
        ...
        if t_max < 10 and r_max < 1.5:
            print("      -> BON (utilisable pour la pipeline)")
        elif t_max < 20 and r_max < 4:
            print("      -> ACCEPTABLE (au plancher de bruit du SO-101)")
        else:
            print("      -> À REVOIR (résidus trop élevés)")
            all_ok = False
    return all_ok, cams


def check_stereo_baseline(cams):
    section("COHÉRENCE STÉRÉO (cam_0 ↔ cam_1)")
    if 0 not in cams or 1 not in cams:
        warn("cam_0 ou cam_1 manquante, baseline non calculée")
        return
    ...
    delta = p1 - p0
    # Angle entre les axes optiques (Z des caméras).
    z0 = T0[:3, 2]
    z1 = T1[:3, 2]
...
def check_perception_modules():
    ...
    for module in ["src.perception.scene",
        ...
        _run_selftest(module)
    # Test d'intégration synthétique.
    result = subprocess.run(
    ...
    if "Tous les tests d'integration passent" in last:
        ...
        warn("tests/perception/test_pipeline.py", f"échoue : {last}")
        print(result.stdout); print(result.stderr)


def check_planning_modules():
    section("MODULES PLANNING")
    for module in ["src.planning.grasp"]:
        _run_selftest(module)


def check_control_modules():
    section("MODULES CONTRÔLE")
...
def _run_selftest(module):
    ...
    if "Tous les tests passent" in last or "tous les tests passent" in last:
        ...
        warn(module, f"self-test échoue : {last}")
        print(result.stdout); print(result.stderr)


def main():
    print()
    print("VALIDATION DE LA CALIBRATION COMPLÈTE — SO-101")
    print()
    ...
    if intr_ok and motor_ok and extr_ok:
        print("  [OK] Calibration validée + modules auto-testés.")
        print("       Prochaine étape : scripts/calibrate_hsv.py puis scripts/run_perception.py")
    else:
        print("  [!] Au moins une étape demande révision (voir détails ci-dessus).")
    print()
