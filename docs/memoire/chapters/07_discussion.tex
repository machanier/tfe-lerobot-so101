...
comme objet de test principal : le score de détection plafonne à $\sim$0{,}05, à
la limite du seuil retenu (0{,}15 pour OWL-ViTv2). Ce score faible dégrade à la
fois la position triangulée et l'orientation estimée du grand axe. Recalibrer le
...
\begin{itemize}
  ...
  \item La pince TPU est compliante et le cylindre de test est rond : si le centre
        ...
        au moment du serrage. C'est l'une des sources de l'unique reprise observée
        lors des essais informels.
\end{itemize}
...
Les résidus \emph{hand-eye} de 5--10~mm pour \texttt{cam\_0}/\texttt{cam\_1}
...
eye-in-hand fait mieux (2{,}48/4{,}77~mm) grâce à un montage plus rigide. Cette
...
explicite trois choix qui méritent d'être justifiés, dont le dernier --- la
faisabilité de la prise latérale --- constitue l'un des résultats négatifs les
plus instructifs du travail.
...
le roundtrip FK$\rightarrow$IK le confirme (erreur $<0{,}5$~mm sur des
configurations aléatoires, $\sim$7~mm sur une pose top-down réelle, soit le
plancher de bruit).
...
\paragraph{Résultat : la prise latérale est mal conditionnée sur 5~DDL.}
...
attrape une canette ? J'ai voulu en avoir le c{\oe}ur net plutôt que de trancher
par intuition. L'hypothèse a donc été testée numériquement, en balayant de l'ordre
de $200\,000$ configurations articulaires et en mesurant, pour chacune, la qualité
d'alignement de la pince avec une fermeture latérale idéale puis l'erreur de
cinématique inverse résultante. Le constat est net : le meilleur alignement
atteint plafonne à $0{,}063$ (sur une échelle où $1$ serait l'alignement parfait) ;
l'IK laisse alors une erreur de $38$ à $60$~mm sur une fermeture horizontale autour
d'un flanc vertical, à comparer aux $<1$~mm obtenus pour un top-down accessible.
Seule une approche frontale strictement radiale s'en approche
($\sim$9~mm, et encore sans convergence franche du solveur), sans offrir le moindre
avantage sur le top-down.

J'insiste sur la formulation, car elle est facile à exagérer : la prise latérale
n'est pas \emph{catégoriquement impossible} sur ce bras, mais elle est
\textbf{mal conditionnée et pas fiable}. Le top-down s'impose donc comme la seule
approche robuste, ce qui borne mécaniquement l'ensemble des objets saisissables
(un objet trop haut, ou trop éloigné en position debout, sort de l'espace
préhensible). Un bras à six degrés de liberté lèverait cette contrainte en rendant
l'orientation d'approche libre. Ce résultat remplace ce qui figurait initialement
comme une piste de travail (« implémenter le side-grasp ») : ce n'est plus une
extension à faire, c'est une limite démontrée du matériel.
...
formelle --- script \texttt{experiment\_campaign.py} --- n'a pas été menée. Seule
une campagne \emph{informelle} a été conduite (cylindre violet, 5 saisies réussies
sur 6, dont une au second essai, couple tenu 270--336, « OBJET TENU » confirmé),
qui ne saurait tenir lieu d'évaluation comparative.}
