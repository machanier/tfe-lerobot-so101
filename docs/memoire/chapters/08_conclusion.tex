...
L'analyse de la prise latérale, conduite numériquement sur environ
$2\times10^{5}$ configurations, a montré qu'une prise horizontale autour d'un
flanc vertical est \emph{mal conditionnée} sur ce bras à 5~DDL (erreur d'IK de
l'ordre de 38 à 60~mm, contre moins de 1~mm pour un top-down accessible) : sans
être catégoriquement impossible, elle n'est pas fiable, ce qui justifie de
retenir le top-down comme approche robuste --- au prix de borner l'ensemble des
objets effectivement saisissables.

\paragraph{Objectif 6 --- Évaluation expérimentale (\emph{partiel}).} C'est la
limite la plus importante à reconnaître. Le système a été éprouvé de façon
\emph{informelle} : sur le cylindre violet, j'ai obtenu 5~saisies réussies sur 6
(dont une au second essai) à travers plusieurs poses (parallèle à $X$, parallèle
à $Y$, en biais à 45\textdegree{}, debout), avec un couple de maintien tenu entre
270 et 336 et la confirmation \og{}objet tenu\fg{}. Mais une campagne
\emph{formelle} et chiffrée --- taux de réussite par classe d'objet, nombre de
replans, comparaison V1/V2/SmolVLA --- reste à mener avec le script dédié.
\todoF{Insérer ici la synthèse chiffrée de la campagne formelle une fois
exécutée (script \texttt{experiment\_campaign.py}). Ne reporter aucune valeur
tant que la campagne n'a pas tourné. La campagne informelle ci-dessus est à
présenter comme préliminaire.}
...
\begin{itemize}
  \item \textbf{Une pipeline modulaire reproductible et auto-testée.} Chaque
        ...
        à moins de 0,5~mm, ou le self-test synthétique de triangulation à moins
        de $10^{-4}$~mm), ce qui permet d'attribuer toute erreur résiduelle à la
  ...
  \item \textbf{L'analyse de la sous-actuation à 5~DDL et de la prise latérale},
        qui transforme une limitation matérielle en résultat documenté : le bras
        atteint de nombreuses orientations dans son plan de tangage mais ne peut
        pas pointer fiablement une approche hors de ce plan, ce qui borne
        rationnellement les stratégies de saisie.
\end{itemize}
