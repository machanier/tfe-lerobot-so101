# Repère base du SO-101 — référence

Ce document explique où se trouve l'origine (0, 0, 0) du repère « base » du
robot, comment mesurer des positions physiques cohérentes, et comment le
vérifier expérimentalement.

À lire avant toute mesure physique servant de ground truth (`gt_test.json`,
`scene.json`, etc.).
...
`base_link` correspond au centre géométrique de la base mécanique du robot,
et non au centre du shoulder_pan motor.
...
Donc en config zéro, la pince est à 39.1 cm devant et 22.6 cm au-dessus du
centre de la plaque de base.
...
géométrique de cette plaque (X=0, Y=0), avec Z=0 au bas de cette plaque (au
niveau de la table).
...
Pour poser un cube (ou tout objet) sur la table devant le robot :
...
Pour un cube de 30 mm posé sur la table, le centre est à Z = +15 mm
(au-dessus de la table = au-dessus de base_link).

## 4. Vérification expérimentale

Pour vérifier que la calibration est correcte et que la mesure part du bon
point :
...
# OU : envoie directement la commande "zero"
python -c "
...
4. Si les mesures correspondent à ±20 mm près : le repère mental et le
   repère URDF coïncident, la calibration est correcte.

5. Si l'écart est plus grand : soit la calibration moteur est décalée
   (refaire `lerobot-calibrate`), soit la calibration hand-eye est biaisée
   (refaire `scripts/solve_handeye_cam.py`), soit le robot n'est pas le
   `new_calib` (refaire la calibration LeRobot complète).

## 5. Position d'une caméra dans le repère base

Les positions des caméras eye-to-hand sont définies par la calibration
hand-eye. On peut les lire :
...
- `cam_2` (eye-in-hand) : c'est `T_gripper_cam`, dépend de la pose du
  bras. La position absolue change en temps réel.

La position 3D détectée d'un objet (par exemple `pos=(+340, +29, +20) mm`
affiché par le pipeline) est dans le repère base (= depuis base_link), et
non depuis une caméra particulière. La triangulation stéréo prend les
images des 2 caméras eye-to-hand et fournit directement la position dans le
repère robot grâce aux matrices `T_base_cam` calibrées.

## 6. Récapitulatif des conventions

| Convention | Valeur |
|---|---|
| Origine (0,0,0) | Centre de la plaque de base inférieure |
| Niveau de la table | Z = 0 mm (= niveau de base_link) |
| Cube 30 mm sur table | Centre à Z = +15 mm |
| Position de l'origine | Au niveau de la table |

Cette lecture rigoureuse de l'URDF fixe la convention de repère.

## 7. Formulation pour le mémoire

Cette section peut alimenter l'annexe ou la section « Architecture >
Repères » :
